...
namespace DocumentManager.ViewModels
{
	...
	public class MainViewModel : ViewModelBase, IDisposable
	{
		...
		private readonly MvxSubscriptionToken _documentAddedToken;
			...
			IMvxMessenger messenger)
		{
			...
			// Subscribe to messages (Messenger pattern)
			_documentAddedToken = _messenger.Subscribe<DocumentAddedMessage>(OnDocumentAdded);
		...
		/// <summary>
		/// Opens the Add Document window - publishes message via Messenger
		/// </summary>
		...
		/// <summary>
		/// Handles DocumentAddedMessage - called when user confirms adding a document
		/// </summary>
		private void OnDocumentAdded(DocumentAddedMessage message)
		{
			try
			{
				var document = _documentService.AddDocument(message.FilePath);
				if (document != null)
				{
		...
		protected virtual void Dispose(bool disposing)
		{
			if (!_disposed)
			{
				if (disposing)
				{
					...
					_messenger.Unsubscribe<DocumentAddedMessage>(_documentAddedToken);
